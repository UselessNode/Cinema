    ...
    public partial class FormHome : Form
    {
        ...
        public void AddRow(DateTime date, int filmID, int occupancy, int roomID)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                ExecuteTransaction(connection, transaction, addingCommand, "Данные добавлены в базу данных");
                LoadTable();
            }
        }
        public void EditRow(DateTime date, int filmID, int occupancy, int roomID)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                SqlTransaction transaction = connection.BeginTransaction();
                SqlCommand editingCommand = new SqlCommand(requestString, connection);
                editingCommand.Parameters.Add("@film", SqlDbType.Int).Value = filmID;
                editingCommand.Parameters.Add("@date", SqlDbType.Date).Value = date;
                editingCommand.Parameters.Add("@room", SqlDbType.Int).Value = roomID;
                editingCommand.Parameters.Add("@occ", SqlDbType.Int).Value = occupancy;
                editingCommand.Parameters.Add("@id", SqlDbType.Int).Value = (int)selectedRow.Cells[0].Value;
                editingCommand.Transaction = transaction;
                ExecuteTransaction(connection, transaction, editingCommand, "Данные изменены");
                LoadTable();
                //try
                //{
                //    editingCommand.ExecuteNonQuery();
                //    transaction.Commit();
        ...
        private void RemoveRow(int id)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                SqlCommand removingCommand = new SqlCommand(requestString, connection);
                removingCommand.Parameters.Add("@id", SqlDbType.Int).Value = id;
                removingCommand.Transaction = transaction;
                ...
                {
                    ExecuteTransaction(connection, transaction, removingCommand, "Данные удалены");
                    LoadTable();
                    //try
                    //{
                    //    removingCommand.ExecuteNonQuery();
                    //    transaction.Commit();
                    //    if (Properties.Settings.Default.ShowNotification)
                    //        MessageBox.Show("Данные изменены", caption: "Транзакция");
                    //    LoadTable();
        ...
        private static void ExecuteTransaction(SqlConnection connection, SqlTransaction transaction, SqlCommand sqlCommand,string headerMessage)
        {
            try
            {
                ...
                transaction.Commit();
                if (Properties.Settings.Default.ShowNotification)
                    MessageBox.Show(headerMessage, caption: "Транзакция");
                connection.Close();
            }
            catch (Exception ex)
            {
